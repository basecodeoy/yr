...
use BaseCodeOy\Yr\Location\Data\Suggest\Response;
...
it('can create a Response instance from response data', function (): void {
    ...
    expect($response->items)->toBeInstanceOf(Collection::class)->and(fn ($items): \Pest\Mixins\Expectation => expect($items)->toHaveCount(2));
...
it('returns an empty collection when no locations are found', function (): void {
    ...
    expect($response->totalResults)->toBe($data['totalResults']);
    expect($response->items)->toBeInstanceOf(Collection::class)->and(fn ($items): \Pest\Mixins\Expectation => expect($items)->toHaveCount(0));
});
